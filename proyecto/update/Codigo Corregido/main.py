...
Módulos utilizados:
    ...
    persistencia → TXT, CSV, Pickle y MySQL (DML completo
...
DB_PATH           = "crisis_monitor"   # nombre de la base de datos MySQL
...
def opcion_cargar_csv(estado):
    ...
    ruta = input("  Ruta del CSV (Enter = 'comentarios_ejemplo.csv'): ").strip()
    if not ruta:
        ruta = "comentarios_ejemplo.csv"
    ...
    try:
        ...
        print(f"    MySQL    → {resultado['mysql_comentarios']} filas | "
              f"{resultado['mysql_alertas']} alertas")
...
def opcion_consultar_db(_estado):
    """Submenú de consultas SELECT a la base de datos MySQL."""
    while True:
        print("\n── CONSULTAR BASE DE DATOS ────────────────────────────────")
        print("  [1] Todos los comentarios (últimos 20)")
        ...
        try:
            ...
            if opcion == "1":
                ...
                print(f"  {'ID':<20} {'USUARIO':<16} {'CATEGORÍA':<14} {'PUNT.':>8}  TEXTO")
                print("  " + "-" * 118)
                for r in registros:
                    usuario = (r.get("usuario") or "N/A")[:16]
                    texto   = r["texto_original"][:80]
                    if len(r["texto_original"]) > 80:
                        texto += "..."
                    print(f"  {str(r['id_original']):<20} {usuario:<16} "
                          f"{r['categoria'].upper():<14} {r['puntuacion']:>8.2f}  {texto}")
                ...
                if idx.isdigit() and 1 <= int(idx) <= len(CATS):
                    ...
                    print(f"  {'ID':<20} {'USUARIO':<16} {'PUNT.':>8}  TEXTO")
                    print("  " + "-" * 110)
                    for r in registros[:15]:
                        usuario = (r.get("usuario") or "N/A")[:16]
                        texto   = r["texto_original"][:85]
                        if len(r["texto_original"]) > 85:
                            texto += "..."
                        print(f"  {str(r['id_original']):<20} {usuario:<16} "
                              f"{r['puntuacion']:>8.2f}  {texto}")
                else:
